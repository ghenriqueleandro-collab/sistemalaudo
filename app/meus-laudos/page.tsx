...
export default function MeusLaudosPage() {
  ...
  const [tipoLaudo, setTipoLaudo] = useState('')   // ← NOVO
  ...
  // ← ATUALIZADO: inclui tipoLaudo no filtro
  const filtrados = useMemo(
    () => {
      let resultado = filtrarLaudos(laudos, { busca, status, cidade, tipoImovel, finalidade })
      if (tipoLaudo) {
        resultado = resultado.filter(
          (l) => ((l as any).tipoLaudo || 'detalhado') === tipoLaudo
        )
      }
      return resultado
    },
  ...
  // ← ATUALIZADO: limpa tipoLaudo também
  function limparFiltros() {
    setBusca(''); setStatus(''); setCidade(''); setTipoImovel(''); setFinalidade(''); setTipoLaudo('')
  }
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 pt-10 lg:px-10 lg:pt-14">
        ...
        <div className="mt-8 rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm lg:p-5 space-y-3">
          <div className="grid gap-3 lg:grid-cols-[repeat(4,minmax(0,1fr))_220px]">
            ...
            <button type="button" onClick={limparFiltros} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">
              Limpar filtros
            </button>
          </div>

          {/* ← NOVO: Filtro por modelo de laudo */}
          <div className="flex items-center gap-2 pt-1">
            <span className="text-xs text-slate-400 font-medium mr-1">Modelo:</span>
            {([
              { value: '',             label: 'Todos' },
        ...
        <div className="mt-8 space-y-3">
          ...
          {filtrados.map((laudo) => {
            ...
            // ← ATUALIZADO: prioriza responsavelNome do laudo, fallback para editorResponsavelNome
            const responsavelNome =
              (laudo as any).responsavelNome ||
              (laudo as any).editorResponsavelNome ||
              null
            ...
            return (
              <div key={laudo.id} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm hover:border-slate-300 transition-colors">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 border-t border-b border-slate-100">
                  ...
                  <div>
                    {/* ← ATUALIZADO: label "Responsável pelo laudo" */}
                    <p className="text-xs text-slate-400 mb-0.5">Responsável pelo laudo</p>
                    {responsavelNome ? (
