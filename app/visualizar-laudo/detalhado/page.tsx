          ...
          <div className="bg-white laudo-conteudo">
            ...
            {(isEvoVis
              ? (evSnap?.elementos || []).map((e: any, idx: number) => {
                  const pnEv2 = (v: any) => { if (!v) return 0; const s = String(v).replace(/[^\d.,]/g,'').replace(',','.'); return parseFloat(s)||0 }
                  const vuCalc = (() => {
                    const calc = evSnap?.resultado?.elementos?.[idx]
                    if (calc?.vu > 0) return calc.vu
                    const aE = pnEv2(e.areaTerreno); const vO = pnEv2(e.valorOferta)
                    const fO = pnEv2(e.fatorOferta) || 1; const bE = e.tipo === 'Terreno c/ benfeitoria' ? pnEv2(e.benfElem) : 0
                    return aE > 0 && vO > 0 ? (vO * fO - bE) / aE : 0
                  })()
                  return { ...e, logradouro: e.logradouro || e.endereco || '', areaTerreno: e.areaTerreno || 0, vu: vuCalc }
                })
              : elemsCddm
            ).map((el: any, i: number) => {
            ...
            {temCddm && cddm && (
              <PaginaFlexivel pagina={proximaPagina()} dataLaudo={dados.dataLaudo}>
                ...
                <div className="mb-8 mt-8">
                  ...
                  <div className="overflow-x-auto mb-4 evitar-quebra">
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '0.5px solid #C9D3E6', fontSize: '11px' }}>
                      <thead>
                        <tr style={{ background: '#1a3564' }}>
                          {['Elem.','VU/m²','F.Local','F.Padrão','F.FOC','F.Andar','F.Vaga','F.Área','VU/m² Hom.'].map((h, i, arr) => (
                            <th key={h} style={{ padding: '4px 4px', fontWeight: 700, color: '#fff', textAlign: 'center', borderRight: i < arr.length - 1 ? '0.5px solid #475e9b' : 'none' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {elemsCddm.map((el: any, i: number) => {
                          const san = el.saneado
                          const td: React.CSSProperties = { padding: '3px 4px', fontSize: '10px', color: san ? '#1e293b' : '#9ca3af', fontStyle: san ? 'normal' : 'italic', textAlign: 'center', borderRight: '0.5px solid #C9D3E6', borderTop: '0.5px solid #C9D3E6' }
                          const fmt4 = (v: number) => (v || 1).toFixed(4).replace('.', ',')
                          return (
                            <tr key={i}>
                              <td style={td}>{i + 1}</td>
                              <td style={td}>{formatarMoeda(el.vu || el.valorUnitarioOferta || 0)}</td>
                              <td style={td}>{fmt4(el.fatorLocal)}</td>
                              <td style={td}>{fmt4(el.fatorPadrao)}</td>
                              <td style={td}>{fmt4(el.fatorFOC)}</td>
                              <td style={td}>{fmt4(el.fatorAndar)}</td>
                              <td style={td}>{fmt4(el.fatorVaga)}</td>
                              <td style={td}>{fmt4(el.fatorArea)}</td>
                              <td style={{ ...td, borderRight: 'none', fontWeight: 700, color: san ? '#17325C' : '#9ca3af' }}>
                  ...
                  {/* Legenda */}
                  <div style={{ background: '#EAF0FB', border: '0.5px solid #C9D3E6', padding: '6px 8px', fontSize: '10px', color: '#1e293b', marginBottom: '12px' }}>
                    <strong>Legenda:</strong> F.Local = Fator de Localização · F.Padrão = Fator de Padrão Construtivo · F.FOC = Obsolescência e Conservação
                    · F.Andar = Fator de Andar · F.Vaga = Fator de Vaga de Garagem · F.Área = Fator de Adequação por Área
                  </div>

                  {/* Memorial + Enquadramento */}
                  <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                    {/* Enquadramento */}
                    <div style={{ flex: 1.3, border: '0.5px solid #C9D3E6' }}>
                      <div style={{ background: '#17325C', padding: '4px 8px', textAlign: 'center' }}>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>ENQUADRAMENTO DA AVALIAÇÃO</span>
                      </div>
                      <div style={{ padding: '6px', fontSize: '10px' }}>
                        <div style={{ display: 'flex', borderBottom: '0.5px solid #C9D3E6', paddingBottom: '4px', marginBottom: '4px' }}>
                          <span style={{ flex: 2 }}>Amplitude do IC de 80%</span>
                          <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column' }}>
                            <div style={{ display: 'flex', gap: '4px', marginBottom: '2px' }}>
                              {['III ≤30%','II ≤40%','I ≤50%'].map(l => <span key={l} style={{ flex: 1, background: '#EAF0FB', textAlign: 'center', padding: '1px', fontWeight: 700, color: '#17325C', fontSize: '9px' }}>{l}</span>)}
                            </div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', borderBottom: '0.5px solid #C9D3E6', padding: '3px 0' }}>
                    ...
                    <div style={{ flex: 1, border: '0.5px solid #C9D3E6' }}>
                      <div style={{ background: '#17325C', padding: '4px 8px', textAlign: 'center' }}>
                        <span style={{ fontSize: '11px', fontWeight: 700, color: '#fff' }}>MEMORIAL DE CÁLCULOS</span>
                      </div>
                      {[
                        ...
                        ['Elementos saneados',     `${elemsCddm.filter((e: any) => e.saneado).length} de ${elemsCddm.length}`],
                        ...
                        <div key={String(l)} style={{ display: 'flex', borderBottom: idx < arr.length - 1 ? '0.5px solid #C9D3E6' : 'none', padding: '3px 6px', fontSize: '10px' }}>
                          <span style={{ flex: 1.5, color: '#1e293b' }}>{l}</span>
                          <span style={{ flex: 1, textAlign: 'right', fontWeight: 700, color: '#2347C6' }}>{v}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '6px', marginBottom: '8px' }}>
                    {[
                      ...
                      <div key={String(l)} style={{ background: '#EAF0FB', border: '0.5px solid #C9D3E6', borderRadius: '3px', padding: '6px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '13px', fontWeight: 700, color: '#17325C' }}>{v}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', border: '0.5px solid #C9D3E6', marginBottom: '6px' }}>
                    <thead>
                      <tr style={{ background: '#1a3564' }}>
                        <th style={{ padding: '4px 6px', fontSize: '11px', fontWeight: 700, color: '#fff', borderRight: '0.5px solid #475e9b' }}>Intervalo</th>
                        <th style={{ padding: '4px 6px', fontSize: '11px', fontWeight: 700, color: '#fff', textAlign: 'center', borderRight: '0.5px solid #475e9b' }}>V.U. (R$/m²)</th>
                        <th style={{ padding: '4px 6px', fontSize: '11px', fontWeight: 700, color: '#fff', textAlign: 'center' }}>Valor total (R$)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ['Mínimo',         cddm.limiteInferior, false],
                        ['Médio (adotado)', cddm.mediaSaneada,   true],
                        ['Máximo',         cddm.limiteSuperior, false],
                        ['Limite −30%',    cddm.limiteInf30,    false],
                        ['Limite +30%',    cddm.limiteSup30,    false],
                      ].map(([lbl, vu, hl]) => {
                        const area = cddm.avaliando?.area || 0
                        const s: React.CSSProperties = { padding: '4px 6px', fontSize: '11px', border: '0.5px solid #C9D3E6', textAlign: 'center', background: hl ? '#EAF0FB' : '#fff', fontWeight: hl ? 700 : 400, color: hl ? '#17325C' : '#1e293b' }
                        return <tr key={String(lbl)}><td style={{ ...s, textAlign: 'left' }}>{String(lbl)}</td><td style={s}>{formatarMoeda(Number(vu))}</td><td style={s}>{formatarMoeda(Number(vu) * Number(area))}</td></tr>
                      })}
                    </tbody>
                  </table>

                  <p style={{ fontSize: '10px', fontStyle: 'italic', color: '#475569' }}>
