...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  <button
    type="button"
    onClick={async () => {
      const { limparLaudoAtual } = await import('@/lib/laudos-storage')
      await limparLaudoAtual()
      window.location.href = '/novo-laudo?modo=novo'
    }}
    className="hidden rounded-full bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow md:inline-flex"
  >
    Novo laudo
  </button>
