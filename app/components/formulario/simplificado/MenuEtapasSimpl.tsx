...
type MenuEtapasSimplProps = {
  etapaAtual: EtapaIdSimpl
  setEtapaAtual: (etapa: EtapaIdSimpl) => void
  etapaConcluida: (etapa: EtapaIdSimpl) => boolean
}

export default function MenuEtapasSimpl({
  etapaAtual,
  setEtapaAtual,
  etapaConcluida,
}: MenuEtapasSimplProps) {
  const indiceAtual = ETAPAS_SIMPL.findIndex((e) => e.id === etapaAtual)
  ...
  const pct = Math.round(((indiceAtual + 1) / ETAPAS_SIMPL.length) * 100)
    ...
    <div className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm">
      ...
      <div className="flex items-center gap-3 px-6 py-2 border-t border-slate-100">
        ...
        <span className="text-xs text-slate-400 tabular-nums shrink-0">
          {indiceAtual + 1} / {ETAPAS_SIMPL.length}
        </span>
      </div>
    </div>
  )
